...
csv_Input.addEventListener('change', function (event) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        reader.onload = function (event) {
            ...
            createCsvButton(csvFileName, csv);
...
function createCsvButton(name, csvFile) {
    console.log(name);
    ...
    csvDataObject[buttonId] = csvFile;
    csvLayerVisibility[buttonId] = false;
...
function addImageOverlayFromHref(href, west, south, east, north, buttonId) {
    ...
    if (!kmlLayerVisibility[buttonId]) {
        map.addLayer(imageOverlay);
        // kmlLayerVisibility[buttonId] = imageOverlay;
    }
}
